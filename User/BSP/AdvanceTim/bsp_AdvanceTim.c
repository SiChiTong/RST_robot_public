...
#include "bsp_AdvanceTim.h" 
  ...
  * @retval 无
  */
static void ADVANCE_TIM8_NVIC_Config(void)
{
    NVIC_InitTypeDef NVIC_InitStructure; 
    // 设置中断组为0
    NVIC_PriorityGroupConfig(NVIC_PriorityGroup_0);		
		// 设置中断来源
    NVIC_InitStructure.NVIC_IRQChannel = ADVANCE_TIM8_IRQ; 	
		// 设置抢占优先级
...
/*--------------------时基结构体初始化-------------------------*/
  ...
  // 使用PWM输入模式时，需要占用两个捕获寄存器，一个测周期，另外一个测占空比
	...
	// 捕获通道IC1配置
	// 选择捕获通道
  TIM_ICInitStructure.TIM_Channel = ADVANCE_TIM8_IC1PWM_CHANNEL;
	// 设置捕获的边沿
  ...
  TIM_ICInitStructure.TIM_ICFilter = 0x0;
	// 初始化PWM输入模式
  TIM_PWMIConfig(ADVANCE_TIM8, &TIM_ICInitStructure);
...
//  TIM_PWMIConfig(ADVANCE_TIM, &TIM_ICInitStructure);
	...
	// 选择输入捕获的触发信号
  TIM_SelectInputTrigger(ADVANCE_TIM8, TIM_TS_TI1FP1);		

	// 选择从模式: 复位模式
	// PWM输入模式时,从模式必须工作在复位模式，当捕获开始时,计数器CNT会被复位
  TIM_SelectSlaveMode(ADVANCE_TIM8, TIM_SlaveMode_Reset);
  TIM_SelectMasterSlaveMode(ADVANCE_TIM8,TIM_MasterSlaveMode_Enable); 

  // 使能捕获中断,这个中断针对的是主捕获通道（测量周期那个）
  TIM_ITConfig(ADVANCE_TIM8, TIM_IT_CC1, ENABLE);	
	// 清除中断标志位
...
static void ADVANCE_TIM1_Mode_Config(void)
{
    ...
    TIM_TimeBaseStructure.TIM_Period=ADVANCE_TIM1_Period;
	  // 时钟预分频数
    TIM_TimeBaseStructure.TIM_Prescaler= ADVANCE_TIM1_Prescaler;	
		// 时钟分频因子 ，没用到不用管
    TIM_TimeBaseStructure.TIM_ClockDivision=TIM_CKD_DIV1;		
		// 计数器计数模式，设置为向上计数
    TIM_TimeBaseStructure.TIM_CounterMode=TIM_CounterMode_Up; 		
